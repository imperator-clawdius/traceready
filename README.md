...
Run `npm run verify:outreach-email` before treating cold-email non-response as market evidence. It checks TraceReady outreach-domain MX, forwarding SPF, DMARC, and DKIM signals, and fails until the domain is ready enough for authenticated outbound email. DNS still cannot prove the `founder@traceready.online` alias exists, so send and receive a manual test, then rerun with `npm run verify:outreach-email -- --alias-tested` before a batch send. Run `npm run score:traction -- --batch docs/proof-led-outreach-batch-02.csv --results private/outreach-results-batch-02.csv --sendability-audit private/outreach-sendability-audit-batch-02.json --contact-recon private/outreach-contact-recon-batch-02.json --output private/traction-readiness-scorecard-2026-06-16.md --today 2026-06-16` to regenerate the private traction-readiness scorecard that separates public proof, send-ready routes, email risk, and real market signals.
