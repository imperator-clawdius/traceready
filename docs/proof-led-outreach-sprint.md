...
```bash
npm run audit:outreach-routes -- --tier importer --limit 15 --json-output private/outreach-route-audit-importer.json
npm run verify:outreach-sendability
...
npm run render:outreach-send-ready -- --results path/to/private-results.csv --sendability-audit private/outreach-sendability-audit-importer.json --route b01-r11 --today 2026-06-16 --output private/send-ready-b01-r11.md
npm run render:outreach-replies -- --results path/to/private-results.csv --route b01-r06 --output path/to/replies-b01-r06.md
npm run update:outreach-result -- --results path/to/private-results.csv --route b01-r06 --date-sent 2026-06-16 --status sent --response-type none --notes "sent via company contact form" --visible-success --next-action "follow up in 4 business days"
...
- Verify the ignored browser route-fit evidence with `npm run verify:outreach-sendability`.
...
- For each queued `browser_form_ready` route, render a final packet with `npm run render:outreach-send-ready -- --results path/to/private-results.csv --sendability-audit private/outreach-sendability-audit-importer.json --route b01-rNN --today 2026-06-16 --output private/send-ready-b01-rNN.md`; do not type into or submit a third-party form without the packet's explicit action-time confirmation.
