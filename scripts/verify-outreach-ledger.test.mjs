...
describe("proof-led outreach ledger verifier", () => {
  it("accepts company-level proof-led outreach rows", () => {
    const rows = parseOutreachLedger(`priority,route_id,tier,company_or_channel,segment,why_it_fits,public_route,source_url,proof_url,file_check_url,message_variant,proof_hook,ask,status,next_step
...
`);
  ...
  it("rejects personal contact data and non-proof-led rows", () => {
    const rows = parseOutreachLedger(`priority,route_id,tier,company_or_channel,segment,why_it_fits,public_route,source_url,proof_url,file_check_url,message_variant,proof_hook,ask,status,next_step
1,b01-r99,importer,Example Coffee,green coffee importer,Example fit,person email,alice@example.com,https://traceready.online/proof/,https://traceready.online/,importer,Generic SaaS pitch,Buy now,not_started,Send first message
